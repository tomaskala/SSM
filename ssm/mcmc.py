...
class MetropolisHastings(abc.ABC):
    ...
    def do_inference(self, y):
        ...
        """
        theta = (
            self.prior.sample(random_state=self.random_state)
            if self.theta_init is None
            else self.theta_init
        )
        ...
        for i in range(self.n_samples):
            ...
            thetas[i] = theta
            print(
                "Done sample {} of {} ({:.02f}%).".format(
                    i + 1, self.n_samples, (i + 1) / self.n_samples * 100.0
                )
            )
            print(
                "Accepted: {} of {} ({:.02f}%) samples so far.".format(
                    accepted, i + 1, accepted / (i + 1) * 100.0
                )
            )
...
class MetropolisHastingsPF(MetropolisHastings, abc.ABC):
    def __init__(
        ...
        super(MetropolisHastingsPF, self).__init__(
            n_samples=n_samples,
            prior=prior,
            proposal=proposal,
...
class MetropolisHastingsABC(MetropolisHastings, abc.ABC):
    ...
    def _alphath_closest(
        self, u: np.ndarray, y_t: Union[float, np.ndarray]
