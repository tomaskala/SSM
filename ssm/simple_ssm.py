...
class ABCSimpleSSM(MetropolisHastingsABC):
    def _transition(self, x: np.ndarray, t: int, theta: np.ndarray) -> np.ndarray:
        V_n = self.random_state.normal(
            loc=0.0, scale=np.sqrt(theta[0]), size=(self.n_particles, 1)
        )
...
class ParticleSimpleSSM(MetropolisHastingsPF):
    ...
    def _observation_log_prob(
        ...
        log_prob = np.sum(
            stats.norm.logpdf(y, np.power(x, 2) / 20, np.sqrt(self.const["sigma2_w"])),
            axis=1,
        )
...
def main():
    ...
    sigma2_w = 1.0
    sigma2_x1 = 0.5
    state_init = stats.norm.rvs(
        loc=0.0,
        scale=np.sqrt(sigma2_x1),
        size=(n_particles, 1),
    ...
    const = {"sigma2_w": sigma2_w}

    prior = Prior([stats.invgamma(a=1.0, scale=10.0)])
